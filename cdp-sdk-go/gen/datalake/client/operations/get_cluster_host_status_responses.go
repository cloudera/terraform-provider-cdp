// Code generated by go-swagger; DO NOT EDIT.

package operations

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"
	"github.com/go-openapi/strfmt"

	"github.com/cloudera/terraform-provider-cdp/cdp-sdk-go/gen/datalake/models"
)

// GetClusterHostStatusReader is a Reader for the GetClusterHostStatus structure.
type GetClusterHostStatusReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *GetClusterHostStatusReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {
	case 200:
		result := NewGetClusterHostStatusOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil
	default:
		result := NewGetClusterHostStatusDefault(response.Code())
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		if response.Code()/100 == 2 {
			return result, nil
		}
		return nil, result
	}
}

// NewGetClusterHostStatusOK creates a GetClusterHostStatusOK with default headers values
func NewGetClusterHostStatusOK() *GetClusterHostStatusOK {
	return &GetClusterHostStatusOK{}
}

/*GetClusterHostStatusOK handles this case with default header values.

Expected response to a valid request.
*/
type GetClusterHostStatusOK struct {
	Payload *models.GetClusterHostStatusResponse
}

func (o *GetClusterHostStatusOK) Error() string {
	return fmt.Sprintf("[POST /api/v1/datalake/getClusterHostStatus][%d] getClusterHostStatusOK  %+v", 200, o.Payload)
}

func (o *GetClusterHostStatusOK) GetPayload() *models.GetClusterHostStatusResponse {
	return o.Payload
}

func (o *GetClusterHostStatusOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.GetClusterHostStatusResponse)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}

// NewGetClusterHostStatusDefault creates a GetClusterHostStatusDefault with default headers values
func NewGetClusterHostStatusDefault(code int) *GetClusterHostStatusDefault {
	return &GetClusterHostStatusDefault{
		_statusCode: code,
	}
}

/*GetClusterHostStatusDefault handles this case with default header values.

The default response on an error.
*/
type GetClusterHostStatusDefault struct {
	_statusCode int

	Payload *models.Error
}

// Code gets the status code for the get cluster host status default response
func (o *GetClusterHostStatusDefault) Code() int {
	return o._statusCode
}

func (o *GetClusterHostStatusDefault) Error() string {
	return fmt.Sprintf("[POST /api/v1/datalake/getClusterHostStatus][%d] getClusterHostStatus default  %+v", o._statusCode, o.Payload)
}

func (o *GetClusterHostStatusDefault) GetPayload() *models.Error {
	return o.Payload
}

func (o *GetClusterHostStatusDefault) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.Error)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
