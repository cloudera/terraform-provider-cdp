// Code generated by go-swagger; DO NOT EDIT.

package operations

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"
	"github.com/go-openapi/strfmt"

	"github.com/cloudera/terraform-provider-cdp/cdp-sdk-go/gen/environments/models"
)

// GetAccountTelemetryReader is a Reader for the GetAccountTelemetry structure.
type GetAccountTelemetryReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *GetAccountTelemetryReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {
	case 200:
		result := NewGetAccountTelemetryOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil
	default:
		result := NewGetAccountTelemetryDefault(response.Code())
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		if response.Code()/100 == 2 {
			return result, nil
		}
		return nil, result
	}
}

// NewGetAccountTelemetryOK creates a GetAccountTelemetryOK with default headers values
func NewGetAccountTelemetryOK() *GetAccountTelemetryOK {
	return &GetAccountTelemetryOK{}
}

/*GetAccountTelemetryOK handles this case with default header values.

Expected response to a valid request.
*/
type GetAccountTelemetryOK struct {
	Payload *models.GetAccountTelemetryResponse
}

func (o *GetAccountTelemetryOK) Error() string {
	return fmt.Sprintf("[POST /api/v1/environments2/getAccountTelemetry][%d] getAccountTelemetryOK  %+v", 200, o.Payload)
}

func (o *GetAccountTelemetryOK) GetPayload() *models.GetAccountTelemetryResponse {
	return o.Payload
}

func (o *GetAccountTelemetryOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.GetAccountTelemetryResponse)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}

// NewGetAccountTelemetryDefault creates a GetAccountTelemetryDefault with default headers values
func NewGetAccountTelemetryDefault(code int) *GetAccountTelemetryDefault {
	return &GetAccountTelemetryDefault{
		_statusCode: code,
	}
}

/*GetAccountTelemetryDefault handles this case with default header values.

The default response on an error.
*/
type GetAccountTelemetryDefault struct {
	_statusCode int

	Payload *models.Error
}

// Code gets the status code for the get account telemetry default response
func (o *GetAccountTelemetryDefault) Code() int {
	return o._statusCode
}

func (o *GetAccountTelemetryDefault) Error() string {
	return fmt.Sprintf("[POST /api/v1/environments2/getAccountTelemetry][%d] getAccountTelemetry default  %+v", o._statusCode, o.Payload)
}

func (o *GetAccountTelemetryDefault) GetPayload() *models.Error {
	return o.Payload
}

func (o *GetAccountTelemetryDefault) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.Error)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
