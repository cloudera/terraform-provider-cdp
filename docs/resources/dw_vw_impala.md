...
- `config` (Attributes) Config to enable sso (see [below for nested schema](#nestedatt--config))
- `enable_unified_analytics` (Boolean) Flag to enable unified analytics.
- `image_version` (String) Image version of the impala.
- `impala_ha_settings` (Attributes) High availability settings for Impala. (see [below for nested schema](#nestedatt--impala_ha_settings))
- `impala_options` (Attributes) Impala-specific configuration options. (see [below for nested schema](#nestedatt--impala_options))
- `impala_query_log` (Boolean) Enable or disable Impala query logging.
...
- `impala_scale_down_delay_seconds` (Number) Scale-down threshold in seconds for Impala.
- `impala_scale_up_delay_seconds` (Number) Scale-up threshold in seconds for Impala.
...
<a id="nestedatt--impala_ha_settings"></a>
### Nested Schema for `impala_ha_settings`
...
<a id="nestedatt--impala_options"></a>
### Nested Schema for `impala_options`
