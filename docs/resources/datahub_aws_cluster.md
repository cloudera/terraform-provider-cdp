...
- `environment` (String) The name or CRN of the environment where the cluster will belong to.
- `name` (String) The name of the cluster. This name must be unique, must have between 5 and 40 characters, and must contain only lowercase letters, numbers and hyphens. Names are case-sensitive.

### Optional

- `architecture` (String) Specifies the CPU architecture of the cluster. Values are ARM64, X86_64.
- `cluster_definition` (String) The name or CRN of the cluster definition.
- `cluster_extension` (Attributes) Cluster extensions for the given Data Hub cluster. (see [below for nested schema](#nestedatt--cluster_extension))
- `cluster_template` (String) The name or CRN of the cluster template.
...
- `image` (Attributes) The details of the image used for cluster instances. (see [below for nested schema](#nestedatt--image))
- `instance_group` (Attributes List) Configurations for instance group (see [below for nested schema](#nestedatt--instance_group))
...
- `subnet_ids` (Set of String) List of subnet IDs in case of multi availability zone setup.
- `tags` (Map of String) Tags to be added to Datahub related resources.
...
- `custom_properties` (String) The custom properties JSON, encoded in base64.
...
- `id` (String) The ID of the image used for cluster instances. This is generated by the Cloudera and uniquely identifies the image in the image catalog.

Optional:

- `catalog` (String) The image catalog name.
...
- `volume_encryption` (Attributes) The volume encryption settings. This setting does not apply to Azure which always encrypts volumes. (see [below for nested schema](#nestedatt--instance_group--volume_encryption))
...
- `encryption` (Boolean) Enable encryption for all volumes in the instance group. Default is false.
- `encryption_key` (String) The ARN of the encryption key to use. If nothing is specified, the default key will be used.
