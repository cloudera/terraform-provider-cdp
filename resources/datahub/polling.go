...
func describeWithRecover(clusterName string, client *client.Datahub, ctx context.Context) (*operations.DescribeClusterOK, error) {
	...
	resp, err := client.Operations.DescribeClusterContext(ctx, operations.NewDescribeClusterParams().WithInput(&datahubmodels.DescribeClusterRequest{ClusterName: &clusterName}))
