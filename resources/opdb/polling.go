...
func describeWithRecover(dataBaseName string, environmentName string, client *client.Opdb, ctx context.Context) (*operations.DescribeDatabaseOK, error) {
	...
	resp, err := client.Operations.DescribeDatabase(operations.NewDescribeDatabaseParams().WithInput(&opdbmodels.DescribeDatabaseRequest{DatabaseName: &dataBaseName, EnvironmentName: &environmentName}))
