...
func (r *privateLinkEndpointResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	switch csp {
	case "AWS":
		if model.AWSAccountDetails.IsNull() || model.AWSAccountDetails.IsUnknown() {
			resp.Diagnostics.AddError("Invalid configuration", "aws_account_details must be provided when cloud_service_provider is \"AWS\"")
			return
		}
		if !model.AzureAccountDetails.IsNull() && !model.AzureAccountDetails.IsUnknown() {
			resp.Diagnostics.AddError("Invalid configuration", "azure_account_details cannot be set when cloud_service_provider is \"AWS\"")
			return
		}
	...
	case "AZURE":
		if model.AzureAccountDetails.IsNull() || model.AzureAccountDetails.IsUnknown() {
			resp.Diagnostics.AddError("Invalid configuration", "azure_account_details must be provided when cloud_service_provider is \"AZURE\"")
			return
		}
		if !model.AWSAccountDetails.IsNull() && !model.AWSAccountDetails.IsUnknown() {
			resp.Diagnostics.AddError("Invalid configuration", "aws_account_details cannot be set when cloud_service_provider is \"AZURE\"")
			return
		}
	...
	params := operations.NewCreatePrivateLinkEndpointParams().WithInput(createReq)
	result, err := r.client.Cloudprivatelinks.Operations.CreatePrivateLinkEndpointContext(ctx, params)
	if err != nil {
	...
	statuses, err := waitForEndpointToBeCreated(trackingID, r.client.Cloudprivatelinks, ctx, nil)
	if err != nil {
		resp.Diagnostics.AddError("Endpoint not ready", err.Error())
		return
	}

	model.EndpointStatuses = fromEndpointStatusesToModel(ctx, statuses)
...
func (r *privateLinkEndpointResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	// No read API exists — write state back unchanged to preserve all fields.
	resp.Diagnostics.Append(resp.State.Set(ctx, &model)...)
}
...
func (r *privateLinkEndpointResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	case "AWS":
		...
		if awsReq.AwsAccountDetails == nil {
			resp.Diagnostics.AddError("Delete failed", "AWS account details are missing from state")
			return
		}
	...
	case "AZURE":
		...
		if azureReq.AzureAccountDetails == nil {
			resp.Diagnostics.AddError("Delete failed", "Azure account details are missing from state")
			return
		}
		cspVal := cloudprivatelinkmodels.CloudServiceProvider(csp)
		deleteReq = &cloudprivatelinkmodels.DeletePrivateLinkEndpointRequest{
	...
	params := operations.NewDeletePrivateLinkEndpointParams().WithInput(deleteReq)
	deleteResult, err := r.client.Cloudprivatelinks.Operations.DeletePrivateLinkEndpointContext(ctx, params)
	if err != nil {
		resp.Diagnostics.AddError("Delete failed", fmt.Sprintf("Error deleting Private Link endpoint: %s", err))
		return
	}

	deleteTrackingID := deleteResult.GetPayload().TrackingID

	tflog.Debug(ctx, "Waiting for Private Link endpoint deletion", map[string]interface{}{"trackingId": deleteTrackingID})
	if err := waitForEndpointToBeDeleted(deleteTrackingID, r.client.Cloudprivatelinks, ctx, nil); err != nil {
		resp.Diagnostics.AddError("Delete not completed", err.Error())
		return
	}
