...
func (r *privateLinkEndpointResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	// No get-by-ID endpoint available; refresh model defaults via fromResponseToModel
	refreshed := fromResponseToModel(ctx, nil)
	refreshed.TrackingID = model.TrackingID
	refreshed.CloudServiceProvider = model.CloudServiceProvider
