...
func convertToAPIImpalaHASettings(model types.Object, ctx context.Context) (*models.ImpalaHASettingsCreateRequest, error) {
	...
	if val, ok := attributes["num_of_active_coordinators"]; ok {
		if value, err := ExtractInt32FromAttribute(ctx, val.(basetypes.Int32Value)); err != nil {
			tflog.Error(ctx, "Error extracting int32 for NumOfActiveCoordinators", map[string]interface{}{"error": err})
			return req, fmt.Errorf("error extracting int32 for NumOfActiveCoordinators: %w", err)
		} else {
	...
	if val, ok := attributes["shutdown_of_coordinator_delay_secs"]; ok {
		if value, err := ExtractInt32FromAttribute(ctx, val.(basetypes.Int32Value)); err != nil {
			...
			return req, fmt.Errorf("error extracting int32 for ShutdownOfCoordinatorDelaySeconds: %w", err)
		} else {
...
func convertToAPIImpalaOptions(model types.Object, ctx context.Context) (*models.ImpalaOptionsCreateRequest, error) {
	...
	if hasScratchSpace && !scratchSpaceLimit.IsUnknown() {
		...
		if err != nil {
			...
			return req, fmt.Errorf("error extracting int32 for ScratchSpaceLimit: %w", err)
		} else {
...
func convertToAPIServiceConfigReq(model types.Object, ctx context.Context) (*models.ServiceConfigReq, error) {
	attributes := model.Attributes()
	...
	if hasEnableSSO && !enableSSOAttr.IsUnknown() {
		...
		if err != nil {
			tflog.Error(ctx, "Error extracting bool for EnableSSO", map[string]interface{}{"error": err})
			return req, fmt.Errorf("error extracting bool for EnableSSO: %w", err)
		} else {
			tflog.Debug(context.Background(), fmt.Sprintf("Assigned value to EnableSSO: %t", req.EnableSSO))
...
func convertToAPIAutoscaling(model types.Object, ctx context.Context) (*models.AutoscalingOptionsCreateRequest, error) {
	...
	if val, ok := attributes["auto_suspend_timeout_seconds"]; ok {
		...
		if err != nil {
			tflog.Error(ctx, "Error extracting int32", map[string]interface{}{"error": err})
			return req, fmt.Errorf("error extracting int32: %w", err)
		} else {
	...
	if val, ok := attributes["impala_scale_down_delay_seconds"]; ok {
		...
		if err != nil {
			tflog.Error(ctx, "Error extracting int32", map[string]interface{}{"error": err})
			return req, fmt.Errorf("error extracting int32: %v", err)
		} else {
	...
	if val, ok := attributes["impala_scale_up_delay_seconds"]; ok {
		intValue, err := ExtractInt32FromAttribute(context.Background(), val)
		if err != nil {
			tflog.Error(ctx, "Error extracting int32 for impala_scale_up_delay_seconds", map[string]interface{}{"error": err})
			return req, fmt.Errorf("error extracting int32 for impala_scale_up_delay_seconds: %w", err)
		} else {
	...
	if val, ok := attributes["max_clusters"]; ok {
		...
		if err != nil {
			...
			return req, fmt.Errorf("error extracting int32 for max_clusters: %w", err)
	...
	if val, ok := attributes["min_clusters"]; ok {
		...
		if err != nil {
			...
			return req, fmt.Errorf("error extracting int32 for min_clusters: %w", err)
		} else {
